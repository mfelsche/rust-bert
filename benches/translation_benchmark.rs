...
use rust_bert::pipelines::generation::private_generation_utils::PrivateLanguageGenerator;
...
fn inputs_tokenization(iters: u64, model: &TranslationModel, data: &[&str]) -> Duration {
    let mut duration = Duration::new(0, 0);
    for _i in 0..iters {
        let start = Instant::now();
        let _ = match model.model {
            Marian(ref model) => {
                model.get_tokenizer().encode_list(
                    data.to_owned(),
                    512 as usize,
...
fn bench_squad(c: &mut Criterion) {
    ...
    c.bench_function("Tokenization pass", |b| {
        b.iter_custom(|iters| black_box(inputs_tokenization(iters, &model, &input)))
    });
